function [D PD] = allfitdist_mod(data,sortby,varargin)
...
hfig=figure;
hold on;
set(hfig, 'units', 'inches', 'Position', [0, 0, 8.5, 11]);
...
%Added here -->        
        indExp = find(strcmp({D.DistName}, 'exponential') == 1);
        if(~any(indExp == inds))
            inds = [inds indExp];
        end
        fid=fopen('res_1100-1149_readlink_crop');
        tline=fgetl(fid);
        indFile=0;

        while ~isempty(tline)
            if(~isempty(str2num(tline)))
                P(indFile+1)=str2num(tline);
                indFile=indFile+1;
            end
            tline=fgetl(fid);
        end
...
%Done adding here -->
        ...
        subplot(2,1,1)
        if lgx
            hplot = semilogx(xi,fi,'k',xi,ys,xi,hyperexpxcdf(xi, Px{:}, MUx{:}));
            set(hplot(1),'linewidth',2);
            set(hplot(end),'linewidth',2);
        else
            hplot = plot(xi,fi,'k',xi,ys,xi,hyperexpxcdf(xi, Px{:}, MUx{:}));
            set(hplot(1),'linewidth',2);
            set(hplot(end),'linewidth',2);
        end
        legend(['empirical',{D(inds).DistName},'hyper-exp'],'Location','NE');
        xlabel('Value');
        ...
        if lgx
            h2plot = semilogx(xi,bsxfun(@minus,ys,fi),xi,bsxfun(@minus,hyperexpxcdf(xi, Px{:}, MUx{:}),fi));
            set(h2plot(end),'linewidth',2);
        else
            h2plot = plot(xi,bsxfun(@minus,ys,fi),xi,bsxfun(@minus,hyperexpxcdf(xi, Px{:}, MUx{:}),fi));
            set(h2plot(end),'linewidth',2);
        end
        ybnds = max(abs(y(:)));
        ax = axis;
        axis([ax(1:2) -ybnds ybnds]);
        legend([{D(inds).DistName},'hyper-exp'],'Location','NE');
        xlabel('Value');
        ylabel('Error');
        title('CDF Error');
        grid on
