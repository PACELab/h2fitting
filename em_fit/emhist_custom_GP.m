function [varargout] = emhist_custom_GP( data, t, w, p_exp,k,s,mu, varargin )
...
%     x = linspace(binctr(1),binctr(end),npts);
    ...
    g = N*binwidth*f;
    u = 10.^x;
    pdfe = u.*pdf('exponential',u,p_exp)*log(10);
    ge=N*binwidth*pdfe;
    pdfb = u.*pdf('Generalized Pareto',u,k,s,mu)*log(10);
    gb=N*binwidth*pdfb;
    ...
    if Display
        ...
        hold on;
        plot(x,sqrt(g),'-','linewidth',1.5,'MarkerSize',9);
        plot(x,sqrt(ge),'-','linewidth',1.5,'MarkerSize',9);
        plot(x,sqrt(gb),'linewidth',1.5);
        xlabel('Log_{10} Interarrival Time');
        ylabel('Count (square root scale)');
        legend('Data','H2','Exp','Gen. Pareto');
    else
        hnd = [];
    end
end
